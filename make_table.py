...
# -------------------------------------------------
# comp_XXX_YYY_kf.csv を読み込み、
#  id(=exp_id), INPUT_MODE, IMG_SIZE, TRAIN_DATASET, MAX_LR ごとに
#  指定メトリック列の mean / max を求め、
#  matplotlib の table で PNG を保存する
# -------------------------------------------------
from __future__ import annotations
...
def parse_args():
    ...
    parser.add_argument(
        "--out",
        default=None,
        help="出力 PNG ファイル名 (省略時: CSV と同じ場所に <stem>_<metric>_table.png)",
    )
    return parser.parse_args()
...
def main():
    ...
    # 必須列チェック
    required_cols = ["INPUT_MODE", "IMG_SIZE", "TRAIN_DATASET_ROOT", "MAX_LR"]
    for col in required_cols:
        if col not in df.columns:
            raise ValueError(f"列 '{col}' が CSV に存在しません。")

    # TRAIN_DATASET_ROOT を短縮形 (最後のパス要素) にしておく
    # 例: /path/to/datasets/type-8_aug-v2 → type-8_aug-v2
    df["TRAIN_DATASET"] = df["TRAIN_DATASET_ROOT"].astype(str).apply(
        lambda s: s.split("/")[-1].split("\\")[-1]
    )

    # --- groupby して mean / max ---
    # ここに TRAIN_DATASET を追加
    group_cols = [id_col, "INPUT_MODE", "IMG_SIZE", "TRAIN_DATASET", "MAX_LR"]
    grouped = (
        df.groupby(group_cols)[metric_col]
        .agg(["mean", "max"])
        .reset_index()
