...
def set_seed(seed: int):
    ...
    # CUDAが利用可能で、適切な環境変数が設定されている場合のみ deterministic algorithms を有効化
...
def load_configs(cfg_path: str | Path) -> List[Dict]:
    ...
    processed = []
    for cfg in cfgs:
        root = Path(cfg.pop("DATASET_ROOT"))
        size = int(cfg["IMG_SIZE"][0])
        mode = cfg["INPUT_MODE"]
        
        # RESIZE_MODE をサポート（デフォルトは "area"）
        resize_mode = cfg.get("RESIZE_MODE", "area")
        
        def build(split: str):
            return str(root / "cache" / mode / f"sz{size}_{resize_mode}" / split / "imgs")

        cfg["TRAIN_DIR"] = build("train")
        cfg["VALID_DIR"] = build("valid")
        cfg["EVAL_DIR"] = cfg["VALID_DIR"]
        cfg["DATASET_ROOT"] = str(root)
        cfg["RESIZE_MODE"] = resize_mode  # 設定に追加
        processed.append(cfg)
    return processed


def make_log_dir(cfg: Dict) -> Path:
    dataset = Path(cfg["DATASET_ROOT"]).name
    size = cfg["IMG_SIZE"][0]
    resize_mode = cfg.get("RESIZE_MODE", "area")
    ...
    run_dir = Path("logs") / dataset / model_tag / mode_size / run_id
...
def run_experiment(cfg):
    ...
    # num_workersを0に変更するだけ
    train_ld = DataLoader(train_ds, cfg["BATCH_SIZE"], True, num_workers=0, generator=g)
    valid_ld = DataLoader(valid_ds, cfg["BATCH_SIZE"], False, num_workers=0, generator=g)
    ...
    with (run_dir / "train_log.csv").open("w", newline="") as f:
        ...
        for ep in range(1, cfg["EPOCHS"] + 1):
            ...
            # ★ 勾配爆発検出を追加
            if ep > 1 and vl > val_losses[-1] * 10:  # 前回の10倍以上
                print(f"⚠️  Gradient explosion detected at epoch {ep}: {vl:.6f}")
                # 必要に応じて学習率を下げるか、前のcheckpointに戻す
    ...
    eval_ld = DataLoader(eval_ds, cfg["BATCH_SIZE"], False, num_workers=0)  # num_workersを0に変更
