...
# 左半分の処理用ハードコードパラメータ
LEFT_BLACK_THRESHOLD = 3      # 「完全な黒」とみなす輝度閾値
LEFT_BRIGHT_SCALE    = 1.2    # 黒以外の画素をどれだけ明るくするか

def enhance_left_half(img: np.ndarray) -> np.ndarray:
    """
    画像の左半分について、
    - 輝度が LEFT_BLACK_THRESHOLD 以下の画素はそのまま
    - それ以外の画素は LEFT_BRIGHT_SCALE 倍にして明るくする
    （RGB / Gray / bin4 いずれにも対応）
    """
    out = img.copy()
    h, w = out.shape[:2]
    half_w = w // 2
    ...
    if left.ndim == 3:
        # RGB の簡易輝度（平均）で判定
        gray = left.mean(axis=2)
    else:
        gray = left.astype(np.float32)

    # 黒以外の画素マスク
    mask = gray > float(LEFT_BLACK_THRESHOLD)

    if left.ndim == 3:
        # 3ch それぞれにスケール
        left_f = left.astype(np.float32)
        left_f[mask] *= float(LEFT_BRIGHT_SCALE)
        left = np.clip(left_f, 0, 255).astype(np.uint8)
    else:
        left_f = left.astype(np.float32)
        left_f[mask] *= float(LEFT_BRIGHT_SCALE)
        left = np.clip(left_f, 0, 255).astype(np.uint8)
...
def hide_half(img: np.ndarray, side: str) -> np.ndarray:
    """
    side: "top" or "bottom" or "right"
    上半分・下半分・右半分のいずれかを黒で隠す。
    左半分を隠すことはしない。
    """
    out = img.copy()
    h, w = out.shape[:2]
    ...
    def get_color(kw: str):
        ...
        if mode == "rgb":
            ...
            if kw == "random_gray":
                ...
                return (v, v, v)
            if kw == "clip":
                # 黒つぶれ or 白飛びを模した輝度のみ使用
                # 0–32 または 224–255 の範囲から選ぶ
                if random.random() < 0.5:
                    v = random.randint(0, 32)      # 黒つぶれ側
                else:
                    v = random.randint(224, 255)   # 白飛び側
                return (v, v, v)
        ...
        if mode in ("gray", "bin4"):
            ...
            if mode == "bin4":
                ...
                if kw == "black":
                    ...
                    # clip: 黒つぶれ or 白飛び領域
                    dark_levels = levels[(levels >= 0) & (levels <= 32)]
                    bright_levels = levels[(levels >= 224) & (levels <= 255)]

                    use_dark = (random.random() < 0.5)

                    if use_dark and len(dark_levels) > 0:
                        v = int(np.random.choice(dark_levels))
                    elif (not use_dark) and len(bright_levels) > 0:
                        v = int(np.random.choice(bright_levels))
                    else:
                        # fallback: 片方しか無い場合など
                        if len(dark_levels) > 0:
                            v = int(np.random.choice(dark_levels))
                        elif len(bright_levels) > 0:
                            v = int(np.random.choice(bright_levels))
                        else:
                            # 両方無い rare case → min/max から
                            if random.random() < 0.5:
                                v = int(levels.min())
                            else:
                                v = int(levels.max())
                ...
                if kw == "black":
                    ...
                    # clip: 黒つぶれ or 白飛び
                    if random.random() < 0.5:
                        v = random.randint(0, 32)      # 黒つぶれ
                    else:
                        v = random.randint(224, 255)   # 白飛び
                elif kw.startswith("random"):
                    v = random.randint(0, 255)
                else:
                    v = 128

            # 1ch の値として返す（fill_roi_partial内で3chに複製）
            return (v,)

        # ── 念のためのフォールバック（未知 mode） ───────────
        # ここにはほぼ来ない想定だが、例外よりは無難な値を返す
        if mode == "rgb":
            return (128, 128, 128)
        else:
            return (128,)
...
def main():
    ...
    for in_dir, modality in target_dirs:
        ...
        # ============================
        # ★ hide 用の領域割り当て（top/bottom/right のいずれか）
        # ============================
        hide_sides = None
        if CONFIG.ENABLE_HIDE:
            n = len(src)
            # 各画像ごとに top / bottom / right のいずれかをランダムに選択
            choices = ["top", "bottom", "right"]
            hide_sides = [random.choice(choices) for _ in range(n)]
