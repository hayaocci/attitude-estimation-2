...
def add_iso_noise(img: np.ndarray, sigma: float) -> np.ndarray:
     """
     Approximate high-ISO sensor noise:
       * Poisson (shot) noise  : signal-dependent
       * Gaussian (read) noise : σ = `sigma`
     """
     if sigma <= 0:
         return img
     img_f = img.astype(np.float32) / 255.0
     # Poisson ― shot noise
     poisson = np.random.poisson(img_f * 255.0) / 255.0
     # Gaussian ― read noise
     gaussian = np.random.normal(0.0, sigma / 255.0, img_f.shape)
     noisy = np.clip(img_f + (poisson - img_f) + gaussian, 0.0, 1.0)
...
def build_augmented_images(img: np.ndarray, roll: float, args, stem: str, out_dir: Path):
    ...
    for pid, ops in BASE_PIPELINES.items():
        ...
        if np.count_nonzero(aug) == 0:          # ① 完全黒ならスキップ
            continue
        cv2.imwrite(str(out_dir / f"{stem}_{pid}.png"), aug)
        ...
        hide_img = hide_quadrants(aug, hide_n_rng)
        if np.count_nonzero(hide_img) != 0:     # ② 完全黒は出力しない
            cv2.imwrite(str(out_dir / f"{stem}_{pid}_{HIDE_SUFFIX}.png"),
                        hide_img)
        ...
        bbox_img = apply_bboxes_full(
            aug,              # img
            roll,             # roll
            pid,              # pipeline id
            num_rng,          # (min,max) random box count
            wh_rng,           # (min,max) width/height
            area_full,        # 塗り潰し可能エリア
            args              # CLI 引数
        )
...
def apply_ops(img, roll, ops: List[str], pid: str, args):
    for op in ops:
        ...
        elif op == "pepper":               # パイプライン名はそのまま
            img = add_iso_noise(img, args.iso_sigma)
        elif op == "bright":
...
def main():
    ...
    ap.add_argument("--iso_sigma", type=float, default=8.0,
                     help="Gaussian σ (8≈ISO1600相当) for ISO noise")
    ap.add_argument("--bright", nargs=2, type=float, default=[0.7, 1.3])
    ...
    ap.add_argument("--crop_scale", nargs=2, type=float, default=[0.3, 0.7])
    args = ap.parse_args()
