#!/usr/bin/env python3
"""
train_dilated_hsv.py ― Research project trainer (Dilated ResNet + HSV version)
===================================================================
* Dilated Convolutionを採用し、パラメータ数を増やさずに受容野を拡大。
* 入力画像を RGB から HSV に変換して学習。
...
class ImageRegressionDataset(Dataset):
    def __init__(self, img_dir: Path | str, img_size: int, normalize: bool = True, use_hsv: bool = True):
        ...
        # HSV/RGB共通の正規化。ToTensor()で[0, 1]になるため、(x-0.5)/0.5 で [-1, 1] に変換
...
def run_experiment(cfg):
    ...
    use_hsv = cfg.get("USE_HSV", True) # YAMLに記載がない場合はデフォルトでTrue
    ...
    print(f"🚀 Starting Experiment: {cfg['id']} (HSV={use_hsv})")
